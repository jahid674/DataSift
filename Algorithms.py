...
def mab_algorithm(partition_data, dataset_name, train_orig, test_orig, Model, target_col, mini_batch_size, max_iteration, tau, budget, alpha):
...
def mab_inf_algorithm(partition_data, dataset_name, train_orig, test_orig, Model, target_col, mini_batch_size, max_iteration, tau, budget, alpha):
    X_train, x_test, y_train_combined, y_test, X_train_orig, X_test_original = prepare_train_data(train_orig, test_orig, target_col)
    y_pred_test=Model.predict_proba(x_test)
    ini_parity= computeFairness(y_pred_test, X_test_original, y_test, 0, dataset_name)
    best_parity = ini_parity       
