...
class RandomForest(nn.Module):
    def __init__(self, input_size, n_estimators=100, learning_rate=0.05):
        super(RandomForest, self).__init__()
        torch.manual_seed(0)
        self.sklearn_rf = RandomForestClassifier(n_estimators=n_estimators, random_state=0)
        self.input_size = input_size
    ...
    def fit(self, x, y):
        ...
        self.sklearn_rf.fit(x, y)

    def predict_proba(self, x):
        self.eval()
        x = x.numpy() if isinstance(x, torch.Tensor) else x
        return self.sklearn_rf.predict_proba(x)[:, 1]

    def predict(self, x):
        proba = self.predict_proba(x)
        return (proba > 0.5).astype(int)
    def partial_fit(self, x, y):
        self.sklearn_rf = clone(self.sklearn_rf)
        self.fit(x, y)


class DecisionTree(nn.Module):
    def __init__(self, input_size, max_depth=None, learning_rate=0.05):
        super(DecisionTree, self).__init__()
        torch.manual_seed(0)
        self.sklearn_dt = DecisionTreeClassifier(max_depth=max_depth, random_state=0)
        self.input_size = input_size
        self.learning_rate = learning_rate
    ...
    def fit(self, x, y):
        x = x.numpy() if isinstance(x, torch.Tensor) else x
        y = y.numpy() if isinstance(y, torch.Tensor) else y
        self.sklearn_dt.fit(x, y)

    def predict_proba(self, x):
        self.eval()
        x = x.numpy() if isinstance(x, torch.Tensor) else x
        return self.sklearn_dt.predict_proba(x)[:, 1]
    ...
    def partial_fit(self, x, y):
        self.sklearn_dt = clone(self.sklearn_dt)
        self.fit(x, y)
